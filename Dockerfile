...
RUN git clone -q https://github.com/richfelker/musl-cross-make.git && \
	...
	sed -i -re 's,([$][(]MAKE[)].*) [$]@,\1 obj_gcc/gcc/.lc_built > /dev/null \&\& find obj_binutils \\( -name "*-new" -o -name "ar" -o -name "nm" -o -name "ranlib" \\) -delete \&\& \1 V=1 install-binutils \&\& &,' musl-cross-make/Makefile && \
...
# Build compiler
# - Use COMMON_CONFIG='CC="gcc -static --static" CXX="g++ -static --static" --disable-shared --enable-static' to build a statically-linked toolchain (https://github.com/richfelker/musl-cross-make/issues/64) to support any Linux distro, but things like LTO will not work correctly
...
# Build libuuid
ARG UTIL_LINUX_GZ_URI=https://github.com/util-linux/util-linux/archive/refs/tags/v2.39.3.tar.gz
